...
page(_($help_context = "Supplier's Group List"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	{
		$input_error = 1;
		display_error(_("The supplier's group name cannot be empty."));
		set_focus('description');
	}

	if ($input_error !=1) {
    	write_supplier_group($selected_id, $_POST['description']);
		if($selected_id != '')
			display_notification(_('Selected group has been updated'));
		else
			display_notification(_('New group has been added'));
		$Mode = 'RESET';
	}
}
...
if ($Mode == 'Delete')
{
	...
	if (supplier_group_used($selected_id))
	{
	...
	{
		delete_supplier_group($selected_id);
...
$result = get_all_supplier_group(check_value('show_inactive'));
...
while ($myrow = db_fetch($result))
{
	...
	inactive_control_cell($code, $myrow["inactive"], 'supplier_group', 'id');
...
if ($selected_id != '') 
{
 	...
 	if ($Mode == 'Edit') {
		...
		$myrow1 = get_supplier_group($selected_id);
...
if ($selected_id != '' && supplier_group_used($selected_id)){
...
text_row(_("Group Name:"), 'description', null, 40, 40);
