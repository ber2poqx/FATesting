...
if (isset($_GET['ModifyOrderNumber']) && is_string($_GET['ModifyOrderNumber'])) {

	$_SESSION['page_title'] = _($help_context = "Modify Purchase Order #") . $_GET['ModifyOrderNumber'];
	create_new_po(ST_PURCHORDER, $_GET['ModifyOrderNumber'], false);
	copy_from_cart();
} elseif (isset($_GET['NewOrder'])) {

	$_SESSION['page_title'] = _($help_context = "Purchase Order Entry");
	create_new_po(ST_PURCHORDER, (string) NULL, false);
	copy_from_cart();
} elseif (isset($_GET['NewGRN'])) {

	$_SESSION['page_title'] = _($help_context = "Receiving Report Entry");
...
if (isset($_GET['AddedID'])) {
	...
	display_note(get_trans_view_str($trans_type, $order_no, _("&View this Draft Purchase Order")), 0, 1);
	...
	//Added by spyrax10
	hyperlink_params("$path_to_root/purchasing/inquiry/po_search_completed.php?", _("Back to Purchase Order Inquiry"), "");
	//
	...
	hyperlink_params(
		"$path_to_root/purchasing/supplier_invoice.php",
		_("Entry purchase &invoice for this receival"),
		"New=1"
	);

	hyperlink_params(
		"$path_to_root/admin/attachments.php",
		_("Add an Attachment"),
	...
	hyperlink_params(
		"$path_to_root/purchasing/supplier_payment.php",
		_("Entry supplier &payment for this invoice"),
...
if (isset($_POST['supplier_id'])) {
	$_POST['StkLocation'] = get_post('Location');
}
//--------------------------------------------------------------------------------------------------
...
function handle_commit_order()
{
	...
	if (can_commit()) {
		...
		if ($cart->order_no == 0 && $cart->trans_type != ST_PURCHORDER) { // new po/grn/invoice
