...
set_page_security( @$_SESSION['PR']->trans_type,
	...
	array(	'NewOrder' => 'SA_PURCHASEREQUEST',
			'ModifyOrderNumber' => 'SA_PURCHASEREQUEST',
...
if (isset($_GET['ModifyOrderNumber']) && is_numeric($_GET['ModifyOrderNumber'])) {
	...
	$_SESSION['page_title'] = _($help_context = "Modify Purchase Request #") . $_GET['ModifyOrderNumber'];
...
function line_start_focus() {
  ...
  function handle_update_item()
  {
	  ...
	  if ($allow_update)
	  {
		  if ($_SESSION['PR']->line_items[$_POST['line_no']]->qty_inv > input_num('qty') ||
			  $_SESSION['PR']->line_items[$_POST['line_no']]->qty_received > input_num('qty'))
		  {
		  ...
		  $_SESSION['PR']->update_order_item($_POST['line_no'], input_num('qty'), input_num('price'),
  ...
  function handle_pr_add_new_item()
  {
	  ...
	  { 
		  ...
		  if ($allow_update == true)
		  {
			  ...
			  if ($allow_update)
			  {
				  $_SESSION['PR']->pr_add_to_order (count($_SESSION['PR']->line_items), $_POST['stock_id'], input_num('qty'), 
					  get_post('stock_id_text'), //$myrow["description"], 
					  input_num('price'), '', // $myrow["units"], (retrived in cart)
					  $_SESSION['PR']->trans_type == ST_PURCHREQUEST ? '' : '', 0, 0);
  ...
  function can_commit()
  {
	  ...
	  {
		if (!check_reference(get_post('ref'), $_SESSION['PR']->trans_type))
		{
			set_focus('ref');
			return false;
		}
	...
	//   if ($_SESSION['PR']->trans_type == ST_PURCHREQUEST && get_post('delivery_address') == '')
	//   {
	...
	// 	  return false;
	//   } 
	//   if (!db_has_currency_rates($_SESSION['PR']->curr_code, $_POST['PRDate'], true))
	// 	  return false;
	//   if ($_SESSION['PR']->order_has_items() == false)
	//   {
	// 	   display_error (_("The order cannot be placed because there are no linesdue_Date entered on this order."));
	// 	   return false;
	//   }
	//   if (floatcmp(input_num('prep_amount'), $_SESSION['PR']->get_trans_total()) > 0)
	//   {
  ...
  function handle_commit_request()
  {
	...
	// 	  copy_to_cart();
	// 	  new_doc_date($cart->orig_order_date);
	// 	  if ($cart->pr_no == 0) { // new po/grn/invoice
