...
function item_settings(&$stock_id, $new_item) 
{
	...
	//check_row(_("Controlled:"), 'controlled',null,true);
	//if(isset($_POST['controlled']) && $_POST['controlled']=1) {
	check_row(_("Serialized:"), 'serialised');
	//}else{
	//	label_row(_('Controlled:'),get_post('controlled'));
	//}
	
	stock_categories_list_row(_("Category:"), 'category_id', null, false, $new_item, $fixed_asset);

	stock_brand_list_row(_('Brand:'), 'brand', null,_('No Brand'));
	stock_manufacturer_list_row(_('Made In:'), 'manufacturer', null);
	stock_distributor_list_row(_('Sub-category:'), 'distributor', null);
	stock_importer_list_row(_('Classification:'), 'importer', null);
