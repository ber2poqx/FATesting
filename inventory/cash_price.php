...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1) 
		{
			...
			update_item_scashprice($selected_id, $_POST['scash_type_id'], $_POST['curr_abrev'], input_num('price'));
			
			//for price archiving
			update_pricehistory($_POST['stock_id'], 0, $_POST['scash_type_id'], 0, 0, 0, 0, 'CSHPRCPLCY');
			add_pricehistory($_POST['stock_id'], input_num('price',0), $selected_id, 0, $_POST['scash_type_id'], 0, 0, 0, 0, 'CSHPRCPLCY', date("Y-m-d H:i:s"));
			
			$msg = _("This cash price has been updated.");
		}
		else
		{
			add_item_scashprice($_POST['stock_id'], $_POST['scash_type_id'], $_POST['curr_abrev'], input_num('price'));
			
			//for price archiving
			$lastInsrtID = db_insert_id();
			update_pricehistory($_POST['stock_id'], 0, $_POST['scash_type_id'], 0, 0, 0, 0, 'CSHPRCPLCY');
			add_pricehistory($_POST['stock_id'], input_num('price',0), $lastInsrtID, 0, $_POST['scash_type_id'], 0, 0, 0, 0, 'CSHPRCPLCY', date("Y-m-d H:i:s"));
