...
page(_($help_context = "Inventory Item Sales Cash prices"), false, false, "", $js);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	elseif ($_POST['scash_type_id'] == 0)
	{
		$input_error = 1;
		display_error(_("Please select cash price code first."));
		set_focus('scash_type_id');
		return false;
	}
	elseif ($_POST['price'] == 0)
	{
   	...
   	elseif ($Mode == 'ADD_ITEM' && get_stock_scashprice_type_currency($_POST['stock_id'], $_POST['scash_type_id'], $_POST['curr_abrev']))
   	{
      	$input_error = 1;
	...
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1) 
		{
			...
			update_item_scashprice($selected_id, $_POST['scash_type_id'], $_POST['curr_abrev'], input_num('price'), date2sql($_POST['date_epic']));
			
			//for price archiving
			update_pricehistory($_POST['stock_id'], 0, $_POST['scash_type_id'], 0, 0, 0, 0, 'CSHPRCPLCY');
			add_pricehistory($_POST['stock_id'], input_num('price',0), $selected_id, 0, $_POST['scash_type_id'], 0, 0, 0, 0, 'CSHPRCPLCY', date("Y-m-d H:i:s"), date2sql(get_post('date_epic')));
			
			$msg = _("This cash price has been updated.");
		}
		else
		{
			add_item_scashprice($_POST['stock_id'], $_POST['scash_type_id'], $_POST['curr_abrev'], input_num('price'), date2sql($_POST['date_epic']));
			
			//for price archiving
			$lastInsrtID = db_insert_id();
			update_pricehistory($_POST['stock_id'], 0, $_POST['scash_type_id'], 0, 0, 0, 0, 'CSHPRCPLCY');
			add_pricehistory($_POST['stock_id'], input_num('price',0), $lastInsrtID, 0, $_POST['scash_type_id'], 0, 0, 0, 0, 'CSHPRCPLCY', date("Y-m-d H:i:s"), date2sql(get_post('date_epic')));
...
if ($Mode == 'Delete')
{
	...
	delete_item_scashprice($selected_id);

	//for price archiving
	remove_id_pricehistory($selected_id, 'CSHPRCPLCY'); //, 'plcycashprice_id'

	display_notification(_("The selected cash price has been deleted."));
	$Mode = 'RESET';
}
...
if (list_updated('stock_id') || isset($_POST['_curr_abrev_update']) || isset($_POST['_scash_type_id_update'])) {
...
$prices_list = get_scashprice($_POST['stock_id']);

div_start('price_table');
start_table(TABLESTYLE, "width='30%'");

$th = array(_("Currency"), _("Price Code"), _("Cash Price"), "E", "D");
table_header($th);
...
while ($myrow = db_fetch($prices_list))
{
    ...
    label_cell(view_price_history($myrow["scash_type"], $myrow["stock_id"]));//modified by albert 05/17/2022
...
if ($Mode == 'Edit')
{
	$myrow = get_stock_scashprice($selected_id);
	$_POST['curr_abrev'] = $myrow["curr_abrev"];
	$_POST['scash_type_id'] = $myrow["scash_type_id"];
...
if (!isset($_POST['price'])) {
	$_POST['price'] = price_format(get_kit_price(get_post('stock_id'), 
		get_post('curr_abrev'),	get_post('scash_type_id')));
}

$kit = get_item_code_dflts($_POST['stock_id']);
small_amount_row(_("Cash Price:"), 'price', null, '', _('per') .' '.$kit["units"]);
