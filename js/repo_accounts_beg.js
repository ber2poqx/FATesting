...
Ext.onReady(function(){
	...
	var ColumnModel = [
		...
		{header:'<b>Customer Name</b>', dataIndex:'name', sortable:true, width:180,
		...
		{header:'<b>Particulars</b>', dataIndex:'comments', sortable:true, width:198,
			renderer: function(value, metaData, record, rowIdx, colIdx, store) {
				metaData.tdAttr = 'data-qtip="' + value + '"';
				return value;
			}
	...
	var tbar = [{
		...
		handler: function(){
			...
			submit_window.show();
			submit_window.setTitle('REPO Beginning Balances - Add');
					...
					items:[{
						...
						listeners: {
							select: function(combo, record, index) {
								...
								ARInvoiceStore.proxy.extraParams = {debtor_id: record.get('debtor_no'), repo_date: Ext.getCmp('repo_date').getValue()};
								ARInvoiceStore.load();
					...
					items:[{
						...
						listeners: {
							select: function(combo, record, index) {
								...
								Ext.getCmp('spotcash').setValue(record.get('lcp_amount'));
