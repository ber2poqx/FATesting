...
// Creator:	RobertGwapo
// date:	2021-04-29
// Title:	Sales Summary By Quantity
// ----------------------------------------------------------------
$path_to_root="..";
...
function print_PO_Report()
{
	...
	$cols = array(0, 55, 110, 170, 240, 300, 360, 430, 490, 
	0);
    ...
    $rep = new FrontReport(_('Sales Summary By Quantity'), "SalesSummaryByQuantity", user_pagesize(), 11, $orientation);
    if ($orientation == 'L')
	...
	while ($SSBA = db_fetch($res))
	{
		...
		$rep->fontSize -= 1;
		$rep->NewLine(1.3);
		$rep->TextCol(0, 1, $SSBA['months_term'] . '  ' . $months);
