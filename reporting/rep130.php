...
function fetch_items($fromsupp, $category=0)
{
		...
		$sql = "SELECT item.stock_id, item.description AS name,
			...
			WHERE item.category_id = category.category_id AND item.stock_id = srp.stock_id";
		if ($category != 0)
			$sql .= " AND category.category_id = ".db_escape($category);

		if ($fromsupp != '')
			$sql .= " AND srp.supplier_id = ".db_escape($fromsupp);
...
function print_price_listing()
{
	...
	while ($myrow=db_fetch($result))
	{
		...
		$price = get_srp_price($myrow['stock_id'], $currency, $salestype);
