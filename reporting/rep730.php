...
function getTransactions($from, $to, $type)
{	
	if ($from != 0)
		$from = date2sql($from);
	...
	if ($from != 0)
	{
		...
		$sql .= " AND gl.type = '$type' ORDER BY bt.receipt_no, gl.`tran_date`, gl.`counter`";
...
function print_Daily_Entries()
{
	...
	$from = $_POST['PARAM_0'];
	$to = $_POST['PARAM_1'];
	$mod_type = $_POST['PARAM_2'];

	$mod_type_name = " - ".$systypes_array[$mod_type];
/*
...
*/
	...
	if($from == 0)
	{
		...
		2 => array('text' => _('Module'), 'from' => $mod_type_name, 'to' => ''),
	...
	$cols = array(5, 40,  90,   125,  225,  315, 495,  535,	0); 
	...
	$aligns = array('left', 'left', 'center', 'left', 'left', 'left', 'right', 'right');

	//$rep = new FrontReport(_('SL Summary (Particulars) - per Transaction'), "SalesSummaryReport", "letter", 9, $orientation = "L");
	$rep = new FrontReport(_('Daily Entries').$mod_type_name, "DailyEntries", "letter", 9, $orientation = "L");
	...
	{		
		...
		$rep->Font('bold');
		$rep->Line($rep->row + 10);
		$rep->TextCol(2, 6, _('- - - - PLEASE SELECT ENTRY MODULE - - -'));
		$rep->Line($rep->row - 5);		
		$rep->End();
		//exit();
	}
	...
	$res = getTransactions($from, $to, $mod_type);
	...
	{	
		...
		$rep->Font('bold');
		$rep->Line($rep->row + 10);
		$rep->TextCol(2, 6, _('- - - - NO TRANSACTION IN THE GIVEN PARAMETER - - -'));
		$rep->Line($rep->row - 5);		
		$rep->End();
		//exit();
	}	
