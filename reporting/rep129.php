...
***********************************************************************/
$page_security = 'SA_SYSCOST_REP'; //Modified by spyrax10 18 Jun 2022
// ----------------------------------------------------------------
// $ Revision:	2.0 $
// Dev.:	Robert Dusal
// date_:	2021-10-12
...
function fetch_array_header_syst_cost($category=0, $supplier)
{
	global $def_coy;
	//$def_coy = 0;
	set_global_connection($def_coy);

	$sql = "SELECT B.cost_type FROM  ".TB_PREF."policy_builder A
				INNER  ".TB_PREF."JOIN supp_cost_types B ON A.cstprctype_id = B.id
			WHERE module_type = 'CSTPLCY'";
	if ($category != 0)
...
function get_sales_type_id_details_syst_cost($category, $supplier)
{
	global $def_coy;
	//$def_coy = 0;
	set_global_connection($def_coy);
...
function get_systemcost_price_item($stock_id, $currency, $cost_type_id, $factor = null, $date = null)
{
	global $def_coy;
	//$def_coy = 0;
	set_global_connection($def_coy);
	...
	$sql = "SELECT price, cost_type_id
		...
		WHERE stock_id = " . db_escape($stock_id) . "
			AND cost_type_id = " . db_escape($cost_type_id);
...
function print_price_listing()
{
	...
	$result1 = fetch_array_header_syst_cost($category, $supplier);
	$columns = array('Category/Items', 'Description', 'UOM');
	//$i=1;
	while ($myrow=db_fetch($result1))
	{
		$columns[] = $myrow['cost_type'];
        //$i++;
	}
	//var_dump($columns);
    ...
    $id_sales_type = array();
    $salestype_id = get_sales_type_id_details_syst_cost($category, $supplier);
    $i = 0; $j = 1;  
	...
	while ($myrow=db_fetch($result))
	{
		...
		$x=3; $y=4; $z=1;
		for($i = 0; $i < $count; $i++) {
			$price = get_systemcost_price_item($myrow['stock_id'], 'PHP', $sales_id[$z]);
