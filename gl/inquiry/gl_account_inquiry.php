...
***********************************************************************/
$page_security = 'SA_GL_LIST'; //Modified by spyrax10 20 Jun 2022
$path_to_root = "../..";
	...
	while ($myrow = db_fetch($result)) {
		...
		if($myrow["type"]==ST_MERCHANDISETRANSFER){
			...
			if ($myrow["person_id"] == '' ) {
				...
				else if (($myrow["type"] == ST_SUPPRECEIVE || $myrow["type"] == ST_SALESINVOICE) && $myrow['account'] == 301003) {
					label_cell($myrow['mcode']);
				}
				else {
			...
			if ($myrow["type"] == ST_BANKPAYMENT || $myrow['type'] == ST_BANKDEPOSIT) {
				...
				if ($myrow["person_name_masterfile"] == '') {
					...
					if (($myrow["type"] == ST_SUPPRECEIVE || $myrow["type"] == ST_SALESINVOICE) && $myrow['account'] == 103003) {
						label_cell($myrow['master_file']);
					}
					else {
