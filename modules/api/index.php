...
// ------------------------------ Branch -----------------------------
$rest->container->singleton('branch', function () {
    return new Branch();
});
$rest->group('/branch', function () use ($rest) {
    // Get Branch
    $rest->get('/', function () use ($rest) {
        $rest->branch->get($rest);
    });
});
// ------------------------------ Branch -----------------------------

// ------------------------------ Receiving Report -----------------------------
$rest->container->singleton('receivingreport', function () {
    return new ReceivingReport();
});
$rest->group('/receivingreport', function () use ($rest) {
    // Get Branch
    $rest->get('/', function () use ($rest) {
    ...
    // Get RR Details
    $rest->get('/details/:id', function ($id) use ($rest) {
        $rest->receivingreport->getReceivingReportDetails($rest, $id);
    });
    // Update invoice qty
