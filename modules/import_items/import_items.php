...
if (isset($_POST['import'])) {
	if (isset($_FILES['imp']) && $_FILES['imp']['name'] != '') {
		...
		while ($data = fgetcsv($fp, 4096, $sep)) {
			if ($lines++ == 0) continue;
			list($type, $code, $id, $description, $category, $units, $qty, $mb_flag, $currency, $price, $brand, $manufacturer, $distributor,$importer) = $data;
			...
			if ($type == 'KIT' || $type == 'FOREIGN') { // Sales Kit or Foriegn Item Code
			    ...
			    if (!$row6) add_item_code($code, $id, $description, $cat, $qty1, $foreign, $brand, $manufacturer, $distributor, $importer);
			    else update_item_code($row6[0], $code, $id, $description, $cat, $qty1, $foreign, $brand, $manufacturer, $distributor, $importer);
			    $k++;
			}
			if ($type == 'ITEM') {
                $dim = 0;
			    ...
			    $row = db_fetch_row($result);
			    if (!$row) {
				    $sql = "INSERT INTO ".TB_PREF."stock_master (stock_id, description, long_description, category_id, tax_type_id, units, mb_flag, sales_account, inventory_account, cogs_account, adjustment_account, wip_account, dimension_id, dimension2_id, brand, manufacturer, distributor, importer)
					    ...
					    '{$_POST['adjustment_account']}', '{$_POST['wip_account']}', $dim, 0, ".db_escape($brand).", ".db_escape($manufacturer).",".db_escape($distributor).",".db_escape($importer).")";
			    ...
			    $row = db_fetch_row($result);
				//add_item_code($code, $id, $description, $cat, 0, $brand, $manufacturer, $distributor, $importer);
				//update_item_code($row[0], $code, $id, $description, $cat, 1, $brand, $manufacturer, $distributor, $importer);
			    if (!$row) add_item_code($code, $id, $description, $cat, 1,0, $brand, $manufacturer, $distributor, $importer);
			    else update_item_code($row[0], $code, $id, $description, $cat, 1,0, $brand, $manufacturer, $distributor, $importer);
			}
