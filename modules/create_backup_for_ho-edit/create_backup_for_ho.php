...
***********************************************************************/
$page_security = 'SA_BACKUP_FOR_HO';
...
if (get_post('view')) {
	if (!get_post('backups')) {
		...
		$filename = $SysPrefs->backup_dir_ho() . clean_file_name(get_post('backups'));
		if (in_ajax()) 
			$Ajax->popup( $filename );
		else {
...
if (get_post('download')) {
	if (get_post('backups')) {
		download_file($SysPrefs->backup_dir_ho().clean_file_name(get_post('backups')));
		exit;
	} else
		display_error(_("Select backup file first."));
}

page(_($help_context = "Backup For Selected Database"), false, false, '', '');
  ...
  global $SysPrefs;
	...
	if (!file_exists($SysPrefs->backup_dir_ho())) {
		display_error (_("Backup paths have not been set correctly.") 
			...
			. _("cannot find backup directory") . " - " . $SysPrefs->backup_dir_ho() . "<br>");
		end_page();
		exit;
	}
}
...
function generate_backup_ho($conn, $ext='no', $comm='')
{
	global $SysPrefs;

	$filename = db_backup_ho($conn, $ext, $comm, $SysPrefs->backup_dir_ho());
	if ($filename)
...
if (get_post('creat')) {
	generate_backup_ho($conn, get_post('comp'), get_post('comments'));
