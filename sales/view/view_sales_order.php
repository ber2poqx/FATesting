...
if ($_GET['trans_type'] == ST_SALESQUOTE) {
	...
	display_heading(sprintf(_("Sales Order Trans #%d"), $_GET['trans_no']));
}
...
$cart = new Cart($_GET['trans_type'], $_GET['trans_no']);
...
label_row(_("Sales Order #"), $_SESSION['View']->reference, "class='tableheader2'", "colspan=3");
label_row(_("Sale's Type"), $_SESSION['View']->stype_name, "class='tableheader2'", "colspan=3");
label_row(_("Co-maker"), 
	normalize_chars(get_comaker($_SESSION['View']->customer_id, $_SESSION['View']->co_maker)), //Modified by spyrax10 7 Feb 2022
...
label_row(
	_("E-mail"),
	"<a href='mailto:" . $_SESSION['View']->email . "'>" . $_SESSION['View']->email . "</a>",
	"class='tableheader2'",
	"colspan=3"
);
...
foreach ($_SESSION['View']->line_items as $stock_item) {
	...
	$line_total = round2(
		$stock_item->quantity * $stock_item->price * (1 - $stock_item->discount_percent),
		user_price_dec()
	);
...
if ($_GET['trans_type'] != ST_SALESQUOTE) {
	...
	start_table(TABLESTYLE, "width='50%' style='margin-top: 10px;'");
	label_row(_("Payment Type"), $cart->payment_policy == 0 ? "CASH" : "INSTALLMENT", "class='tableheader2'");
	if ($cart->payment_policy != 0) {
		label_row(_("1st DownPay"), number_format($cart->dp_amount), "class='tableheader2'");
    	label_row(_("First Due Date"), $cart->first_due_date, "class='tableheader2'");
    	label_row(_("Maturity Date"), $cart->maturity_date, "class='tableheader2'");
	}
    
	if ($cart->payment_policy != 0) {
		label_row(_("Months Term"), $cart->months_term, "class='tableheader2'");
		label_row(_("Rebate"), $cart->rebate, "class='tableheader2'");
		label_row(_("Financing Rate"), $cart->financing_rate . "%", "class='tableheader2'");
		label_row(_("LCP Amount"), number_format($cart->lcp_amount), "class='tableheader2'");
		label_row(_("Due/Amortization"), number_format($cart->amortization), "class='tableheader2'");
	} else {
		label_row(_("Total LCP Amount"), number_format($cart->lcp_amount), "class='tableheader2'");
	}
	label_row(_("A/R Amount"), number_format($cart->ar_amount), "class='tableheader2'");
