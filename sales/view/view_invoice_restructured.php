...
page(_($help_context = "View Sales Invoice Restructured"), true, false, "", $js);
...
$header = get_sales_invoice_restructured_header($trans_id);
...
$amortization_schedule = get_deptor_loan_schedule_ob($trans_id, $header["debtor_no"], ST_RESTRUCTURED);
...
comments_display_row(ST_RESTRUCTURED, $trans_id);
end_table();
...
// $tax_items = get_trans_tax_details(ST_RESTRUCTURED, $trans_id);
...
$voided = is_voided_display(ST_RESTRUCTURED, $trans_id, _("This invoice has been voided."));

if (!$voided) {
	display_allocations_to(PT_CUSTOMER, $myrow['debtor_no'], ST_RESTRUCTURED, $trans_id, $myrow['Total']);
}
...
end_page(true, false, false, ST_RESTRUCTURED, $trans_id);
