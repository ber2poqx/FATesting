...
//
$path_to_root = "..";
$page_security = 'SA_SALESINVOICEREPO';

include_once($path_to_root . "/sales/includes/cart_class.inc");
include_once($path_to_root . "/includes/session.inc");
include_once($path_to_root . "/sales/includes/sales_ui.inc");
include_once($path_to_root . "/sales/includes/ui/si_repo_install_ui.inc");
...
set_page_security(
	...
	array(
		...
		'AddedDIREPO' => 'SA_SALESINVOICEREPO',
...
if (isset($_GET['NewDelivery']) && is_numeric($_GET['NewDelivery'])) {
	...
	if (isset($_GET['FixedAsset'])) {
		...
		$_SESSION['page_title'] = _($help_context = "Sales Invoice Repo Installment");
	...
	$_SESSION['page_title'] = _($help_context = "New Sales Order Repo Entry");
...
if (isset($_GET['AddedID'])) {
	...
	submenu_option(_("Enter a &New Order"),	"/sales/si_repo_install.php?NewOrder=0");
	...
	submenu_option(
		...
		"/sales/si_repo_install.php?NewQuoteToSalesOrder=$order_no"
	);

	submenu_option(_("Enter a New &Quotation"),	"/sales/si_repo_install.php?NewQuotation=0");
	...
	if ((isset($_GET['Type']) && $_GET['Type'] == 1))
		...
		submenu_option(
			...
			"/sales/si_repo_install.php?NewDelivery=0"
		);

	display_footer_exit();
} elseif (isset($_GET['AddedDIREPO'])) {
	$invoice = $_GET['AddedDIREPO'];

	display_notification_centered(sprintf(_("Invoice # %d has been entered."), $invoice));

	submenu_view(_("&View This Invoice Repo"), ST_SALESINVOICEREPO, $invoice);

	submenu_print(_("&Print Sales Invoice Repo"), ST_SALESINVOICEREPO, $invoice . "-" . ST_SALESINVOICEREPO, 'prtopt');
	submenu_print(_("&Email Sales Invoice Repo"), ST_SALESINVOICEREPO, $invoice . "-" . ST_SALESINVOICEREPO, null, 1);
	set_focus('prtopt');
	...
	submenu_view(_("&View This Invoice Restructured Repo"), ST_RESTRUCTURED, $invoice);
	...
	display_note(get_gl_view_str(ST_RESTRUCTURED, $invoice, _("View the GL &Journal Entries for this Invoice Restructured Repo")), 0, 1);
...
function can_process()
{
	...
	$row = get_DL_by_reference(get_post('document_ref'), ST_SALESINVOICEREPO);
	...
	if ($_SESSION['Items']->trans_type == ST_SALESINVOICEREPO && date('Y,m,d', strtotime(get_so_date($_SESSION['Items']->order_no))) >  date('Y,m,d', strtotime(get_post('OrderDate')))) {
	...
	// if ($_SESSION['Items']->trans_type == ST_SITERMMOD && 
	// 	advance_payment($row['trans_no'], ST_SALESINVOICEREPO, $row['debtor_no'], date2sql(get_post('OrderDate'))) > 0) {
...
if (isset($_POST['ProcessOrder']) && can_process()) {
	...
	if ($ret == -1) {
		...
		if ($modified) {
			...
			meta_forward($_SERVER['PHP_SELF'], "AddedDIREPO=$trans_no&Type=$so_type");
...
function installment_computation()
{
	...
	if ($terms > 1) {
		...
		if($terms == 1){
			...
			$mature_date = add_months($_POST['OrderDate'], 1);
			$_POST['maturity_date'] = add_months($mature_date, 0);

		}else{
			if($terms > 0 && $terms < 1){
				$_POST['first_due_date'] = add_days($_POST['OrderDate'], 15);
				$mature_date = add_days($_POST['OrderDate'], 15);
				$_POST['maturity_date'] = $mature_date;
			}
		}		
	}
	...
	/* */
		...
		/*full payment term computation Added by Albert 09/28/2022*/
			...
			if($terms == 3)
			{
				...
				$_POST['amount_to_be_paid'] = round($total_amount_due - get_post('alloc') + get_post('rebate_if_adv_pay'));
...
function  handle_cancel_order()
{
	...
	if ($_SESSION['Items']->trans_type == ST_CUSTDELIVERY) {
		...
		submenu_option(_("Enter a New Sales Delivery"),	"/sales/si_repo_install.php?NewDelivery=1");
		...
		submenu_option(_("Enter a New Sales Invoice"),	"/sales/si_repo_install.php?NewInvoiceRepo=1");
	} elseif ($_SESSION['Items']->trans_type == ST_SALESQUOTE) {
		if ($_SESSION['Items']->trans_no != 0)
			delete_sales_order(key($_SESSION['Items']->trans_no), $_SESSION['Items']->trans_type);
		display_notification(_("This sales quotation has been cancelled as requested."), 1);
		submenu_option(_("Enter a New Sales Quotation"), "/sales/si_repo_install.php?NewQuotation=Yes");
	} else { // sales order
		if ($_SESSION['Items']->trans_no != 0) {
			...
			if (sales_order_has_deliveries($order_no)) {
				...
				submenu_option(_("Enter a New Sales Order"), "/sales/si_repo_install.php?NewOrder=Yes");
			}
		} else {
			processing_end();
