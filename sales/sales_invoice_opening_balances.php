...
//
$path_to_root = "..";
$page_security = 'SA_SALESINVOICEOB';
...
if (isset($_GET['NewDelivery']) && is_numeric($_GET['NewDelivery'])) {
	...
	if (isset($_GET['FixedAsset'])) {
		...
		$_SESSION['page_title'] = _($help_context = "Sales Invoice for Opening Balances");
...
if (isset($_GET['AddedID'])) {
	...
	submenu_option(_("Enter a &New Order"),	"/sales/sales_invoice_opening_balances.php?NewOrder=0");
	...
	submenu_option(_("Make &Sales Order Against This Quotation"),
		"/sales/sales_invoice_opening_balances.php?NewQuoteToSalesOrder=$order_no");

	submenu_option(_("Enter a New &Quotation"),	"/sales/sales_invoice_opening_balances.php?NewQuotation=0");
	...
	if ((isset($_GET['Type']) && $_GET['Type'] == 1))
		...
		submenu_option(_("Enter a &New Delivery"), 
			"/sales/sales_invoice_opening_balances.php?NewDelivery=0");
	...
	if ((isset($_GET['Type']) && $_GET['Type'] == 1))
		...
		submenu_option(_("Enter a &New Sales Invoice Opening Balances"),
			"/sales/sales_invoice_opening_balances.php?NewInvoice=0");
...
function copy_to_cart()
{
	...
	$cart->deferred_gross_profit = $_POST['deferred_gross_profit'];//09-13/2021 added by albert
	$cart->profit_margin = $_POST['profit_margin'];//09-13/2021 added by albert
	$cart->old_trans_no = $_POST['old_trans_no'];//Added by Albert
	$cart->lcp_amount = $_POST['lcp_amount'];
	$cart->ob_standard_cost = $_POST['ob_standard_cost']; //modified by spyrax10
	...
	$cart->ob_total_amount_paid = $_POST['total_amount_paid'];
}
...
function copy_from_cart()
{
	...
	$_POST['deferred_gross_profit'] = $cart->deferred_gross_profit;//09-13/2021 added by albert
	$_POST['total_amount_paid'] = $cart->ob_total_amount_paid;
}
//--------------------------------------------------------------------------------
...
function can_process() {
	...
	//Added by Albert
	if (get_ob_old_trans_no($_POST['old_trans_no'])){
		display_error(_("Old trans no has already use."));
		set_focus('old_trans_no');
		return false;
...
if (isset($_POST['ProcessOrder']) && can_process()) {
	...
	$_SESSION['Items']->prepaid = true;
	$ret = $_SESSION['Items']->write(1, 0, 1, get_post('waranty_code'), get_post('fsc_series'), 
		input_num(get_post('ob_standard_cost'))); //Added by spyrax10
...
{
	$price = $_SESSION['Items']->get_items()[0]->price;
	...
	// $amort = ($diff_lcp_downpayment * $quotient_financing_rate) + $diff_lcp_downpayment / $terms + $rebate;
	...
	// display_error(_("Terms: " . $terms));
	// display_error(_("Quotitent Finance Rate: " . $quotient_financing_rate));
	// display_error(_("LCP: " . floatval($_POST['lcp_amount'])));
	// display_error(_("Down: " . floatval($_POST['down_pay'])));
	// display_error(_("Amount to be Finance: " . $amount_to_be_finance));
	// display_error(_("Rebate: " . $rebate));
	...
	$_POST['maturity_date'] = add_months($_POST['first_due_date'], $terms);
	$_POST['rebate'] = $rebate;
...
function check_item_data()
{
	...
	//Added by spyrax10
	if (is_Serialized($_POST['stock_id']) == 1 && $_POST['serialeng_no'] == '') {
		display_error(_("Serial Number cannot be empty for this item!"));
		set_focus('serialeng_no');
		return false;
	}

	if (get_stock_catID($_POST['stock_id']) == 14 && $_POST['chassis_no'] == '') {
		display_error(_("Chassis Number cannot be empty for this item!"));
		set_focus('chassis_no');
		return false;
	}

	if (get_stock_catID($_POST['stock_id']) == 14 && $_POST['color_desc'] == '') {
		display_error(_("Color description can't be empty for this item!"));
		set_focus('color_desc');
		return false;
	}

	if (get_stock_catID($_POST['stock_id']) == 14 && get_color_description_opening($_POST['color_desc'], $_POST['stock_id']) == false) {
		display_error(_("Color code doesn't match to this item!"));
		set_focus('color_desc');
		return false;
	}

	if (is_Serialized($_POST['stock_id']) == 1 && $_POST['qty'] == 0) {
		display_error(_("Quantity can't be zero!"));
		set_focus('qty');
		return false;
	}

	if (is_Serialized($_POST['stock_id']) == 1 && $_POST['qty'] > 1) {
	...
	if ($_POST['price'] == 0) {
		display_error(_("Price can't be zero!"));
		set_focus('price');
		return false;
	}
	//
...
function  handle_cancel_order()
{
	...
	if ($_SESSION['Items']->trans_type == ST_CUSTDELIVERY) {
		...
		submenu_option(_("Enter a New Sales Delivery"),	"/sales/sales_invoice_opening_balances.php?NewDelivery=1");
		...
		submenu_option(_("Enter a New Sales Invoice"),	"/sales/sales_invoice_opening_balances.php?NewInvoice=1");
	} elseif ($_SESSION['Items']->trans_type == ST_SALESQUOTE)
	{
		if ($_SESSION['Items']->trans_no != 0) 
			delete_sales_order(key($_SESSION['Items']->trans_no), $_SESSION['Items']->trans_type);
		display_notification(_("This sales quotation has been cancelled as requested."), 1);
		submenu_option(_("Enter a New Sales Quotation"), "/sales/sales_invoice_opening_balances.php?NewQuotation=Yes");
	} else { // sales order
		if ($_SESSION['Items']->trans_no != 0) {
			...
			if (sales_order_has_deliveries($order_no))
			{
				...
				submenu_option(_("Enter a New Sales Order"), "/sales/sales_invoice_opening_balances.php?NewOrder=Yes");
			}
		} else {
			processing_end();
