...
if (isset($_GET['NewDelivery']) && is_numeric($_GET['NewDelivery'])) {
	...
	if (isset($_GET['FixedAsset'])) {
		...
		$_SESSION['page_title'] = _($help_context = "Sales Invoice Cash");
	...
	$_SESSION['page_title'] = _($help_context = "New Sales Order Cash");
...
function copy_from_cart()
{
	...
	$_POST['pdc_no'] = $cart->pdc_no;
	$_POST['pdc_discount'] = $cart->pdc_discount;
}
//--------------------------------------------------------------------------------
	...
	if ($_SESSION['Items']->trans_type != ST_SALESINVOICE && ($_POST['category_id'] == 23 || $_POST['category_id'] == 18)) {
		...
		if (get_post('pdc_chk') == 1) {
			if (get_post('pdc_no') == '') {
				...
				display_warning(_("With PDC Number is selected! Please Double Check your Discount Input.."));
			}
