...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error !=1) {
    	write_sale_reason($selected_id, $_POST['description']);
		if($selected_id != '')
...
if ($Mode == 'Delete')
{
	delete_sale_reason($selected_id);
...
$result = get_all_sales_reason(check_value('show_inactive'));
...
while ($myrow = db_fetch($result))
{
	...
	inactive_control_cell($code, $myrow["inActive"], 'sales_reason', 'id');
...
if ($selected_id != '') 
{
 	...
 	if ($Mode == 'Edit') {
		...
		$myrow1 = get_sale_reason($selected_id);

		$_POST['id'] = $myrow1["id"];
		$_POST['description']  = $myrow1["reason"];
