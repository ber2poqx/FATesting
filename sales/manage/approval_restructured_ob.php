...
$path_to_root = "../..";
$page_security = 'SA_SALES_RESTRUCTURED_APPROVAL';
...
page(_("Draft for SI Restructured Opening Balances #" . $_GET['trans_no']), false, false, "", $js);
...
function get_SIOB_header($trans_no) {
    ...
    $result = db_query($sql, "No Items return for SI Opening Balances!");
...
function get_SIOB_reference($trans_no) {
    ...
    $result = db_query($sql, "Cant get SI Opening Balances reference! ");
    set_global_connection();
	$row = db_fetch_row($result);
	return $row[0];
}

function update_SIOB_status($trans_no, $status,$aprroval_remarks) {

    global $Refs;

    $sql = "UPDATE ".TB_PREF."debtor_trans 
        SET restructured_status = '$status', approval_remarks = '$aprroval_remarks'
        WHERE type != 13 AND opening_balances = 1 
            AND trans_no = " .db_escape($trans_no);

    set_global_connection();
    db_query($sql, "Cannot update SI Opening Balances status!");
...
if (isset($_POST['Approved']) && can_proceed()) {
    update_SIOB_status($_GET['trans_no'], 1, $_POST['Comments']);
    meta_forward("../sales_invoice_ob_list.php?");
}

if (isset($_POST['Disapproved']) && can_proceed()) {
    update_SIOB_status($_GET['trans_no'], 2, $_POST['Comments']);
    meta_forward("../sales_invoice_ob_list.php?");
}
//-----------------------------------------------------------------------------
