		...
		if (isset($_FILES['impCSVS']) && $_FILES['impCSVS']['name'] != '') {
			...
			if (!$fp)
				...
				while ($data = fgetcsv($fp, 4096, $sep)) {
					...
					$address = normalize_chars($address);
					$barangay = normalize_chars($barangay);
					$province = normalize_chars($province);
					$spouse = normalize_chars($spouse);
					$name_father = normalize_chars($name_father);
					$name_mother = normalize_chars($name_mother);
					$email = normalize_chars($email);
					$facebook = normalize_chars($facebook);
