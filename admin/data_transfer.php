...
if(isset($_GET['submit'])){
    ...
    if($InputError !=1) {
        if($_POST['datadesc'] == "item"){
            ...
            //foreach($_POST['branch'] as $value=>$data)
            //{
                $branch = $_POST['branch'];
                ...
                while ($myrow = db_fetch($result)) 
                {
                    ...
                    $checkres = check_item($myrow["stock_id"], $branch);
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkrescode = check_item_codes($myrow["item_code"], $branch);
                    ...
                    if ($counter > 0) {
                        ...
                        $myrow["color"], $myrow["old_code"], $myrow["date_modified"], $branch);
                    }else{
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkplcy = check_plcy_code($myrow["plcy_code"], $branch);
                    ...
                    if ($counter == 0) {
                        ...
                        $myrow["module_type"], $myrow["inactive"], $myrow["category_id"], $myrow["branch_area_id"],  $branch);
                    }
                }
                ...
                while ($myrow1 = db_fetch($result1)) {
                    ...
                    $checkplcydtls = check_plcydetails_code($myrow1["plcydtl_code"], $branch);
           ...
           $dsplymsg = _("The Policy has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checksuppl = check_supplier($myrow["supplier_id"], $branch);
                    ...
                    if ($counter > 0) {
                        ...
                        $myrow["SAPcode"], $myrow["supplier_type"], $branch);
                    }else{
           ...
           $dsplymsg = _("The Supplier has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkbrand = check_brand($myrow["id"], $branch);
                    while ($checkrow = db_fetch($checkbrand)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_brand($myrow["id"], $myrow["code"], $myrow["name"], $myrow["cat_id"], $myrow["inactive"], $branch);
                    }else {
                        add_to_brand($myrow["code"], $myrow["name"], $myrow["cat_id"], $myrow["inactive"], $branch);
                    }
                }
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
           $dsplymsg = _("The Brand has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkclass = check_class($myrow["id"], $branch);
                    while ($checkrow = db_fetch($checkclass)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_classification($myrow["id"], $myrow["code"], $myrow["name"], $myrow["inactive"], $branch);
                    }else{
                        add_to_classification($myrow["code"], $myrow["name"], $myrow["inactive"], $branch);
                    }
                }
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
           $dsplymsg = _("The Classification has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checksubcateg = check_sub_categ($myrow["id"], $branch);
                    while ($checkrow = db_fetch($checksubcateg)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_subcategory($myrow["id"], $myrow["code"], $myrow["name"], $myrow["inactive"], $branch);
                    }else{
                        add_to_subcategory($myrow["code"], $myrow["name"], $myrow["inactive"], $branch);
                    }
                }
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
           $dsplymsg = _("The Sub-category has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkgl = check_glaccount($myrow["account_code"], $branch);
                    ...
                    if ($counter > 0) {
                        ...
                        $myrow["control"], $myrow["normal_balance"], $branch);
                    }else {
           ...
           $dsplymsg = _("The Gl Account has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkbank = check_bankaccount($myrow["id"], $branch);
                    ...
                    if ($counter > 0) {
                        ...
                        $myrow["ending_reconcile_balance"], $myrow["inactive"], $branch);
                    }else {
           ...
           $dsplymsg = _("The Bank Account has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkgroup = check_groupaccount($myrow["id"], $branch);
                    while ($checkrow = db_fetch($checkgroup)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_acountgrp($myrow["id"], $myrow["name"], $myrow["class_id"], $myrow["parent"], $myrow["inactive"], $branch);
                    }else {
                        add_to_accountgrp($myrow["id"], $myrow["name"], $myrow["class_id"], $myrow["parent"], $myrow["inactive"], $branch);
                    }
                }
                ...
                while ($myrow1 = db_fetch($result1)) {
                    ...
                    $checkclass = check_classaccount($myrow1["cid"], $branch);
                    while ($checkrow = db_fetch($checkclass)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_classaccount($myrow1["cid"], $myrow1["class_name"], $myrow1["ctype"], $myrow1["inactive"], $branch);
                    }else {
                        add_to_classaccount($myrow1["cid"], $myrow1["class_name"], $myrow1["ctype"], $myrow1["inactive"], $branch);
                    }
                }
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
           $dsplymsg = _("The GL Group/Classes Account has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkcost = check_systemcost($myrow["id"], $branch);
                    while ($checkrow = db_fetch($checkcost)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_systemcost($myrow["id"], $myrow["cost_type"], $myrow["tax_included"], $myrow["factor"], $myrow["inactive"], $branch);
                    }else {
                        add_to_systemcost($myrow["cost_type"], $myrow["tax_included"], $myrow["factor"], $myrow["inactive"], $branch);
                    }
                }
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
           $dsplymsg = _("The System Cost has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkuser = check_datauser($myrow["user_id"], $branch);
                    ...
                    if ($counter > 0) {
                        ...
                        $myrow["def_print_orientation"], $myrow["inactive"], $myrow["passupdate"], $branch);
                    }else {
                        ...
                        $myrow["def_print_destination"], $myrow["def_print_orientation"], $myrow["inactive"], $myrow["passupdate"], $branch);
                    }
                }
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
           $dsplymsg = _("The User has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkslprc = check_salesprice($myrow["id"], $branch);
                    while ($checkrow = db_fetch($checkslprc)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_salesprice($myrow["id"], $myrow["sales_type"], $myrow["remarks"], $myrow["tax_included"], $myrow["factor"], $myrow["inactive"], $branch);
                    }else {
                        add_to_salesprice($myrow["sales_type"], $myrow["remarks"], $myrow["tax_included"], $myrow["factor"], $myrow["inactive"], $branch);
                    }
                }
                ...
                while ($myrow1 = db_fetch($result1)) {
                    ...
                    $checkcash = check_cashprice($myrow1["id"], $branch);
                    while ($checkrow = db_fetch($checkcash)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_cashprice($myrow1["id"], $myrow1["scash_type"], $myrow1["tax_included"], $myrow1["factor"], $myrow1["inactive"], $branch);
                    }else {
                        add_to_cashprice($myrow1["scash_type"], $myrow1["tax_included"], $myrow1["factor"], $myrow1["inactive"], $branch);
                    }
                }
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
           $dsplymsg = _("The Sales Price/Cash Price Setup has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checksltype = check_salestype($myrow["id"], $branch);
                    while ($checkrow = db_fetch($checksltype)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_salestype($myrow["id"], $myrow["code"], $myrow["name"], $myrow["inactive"], $branch);
                    }else {
                        add_to_salestype($myrow["code"], $myrow["name"], $myrow["inactive"], $branch);
                    }
                }
                ...
                while ($myrow1 = db_fetch($result1)) {
                    ...
                    $checkreason = check_salesreason($myrow1["id"], $branch);
                    while ($checkrow = db_fetch($checkreason)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_salesreason($myrow1["id"], $myrow1["reason"], $myrow1["inActive"], $branch);
                    }else {
                        add_to_salesreason($myrow1["reason"], $myrow1["inActive"], $branch);
                    }
                }
                ...
                while ($myrow2 = db_fetch($result2)) {
                    ...
                    $checkclctype = check_collectiontype($myrow2["collect_id"], $branch);
                    while ($checkrow = db_fetch($checkclctype)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_collectiontype($myrow2["collect_id"], $myrow2["collection"], $myrow2["inactive"], $branch);
                    }else {
                        add_to_collectiontype($myrow2["collection"], $myrow2["inactive"], $branch);
                    }
                }
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
           $dsplymsg = _("Sales Type/Reason & Collection Type has been successfully entered...");
            ...
            //{
                ...
                while ($myrow = db_fetch($result)) {
                    ...
                    $checkcredit = check_credit($myrow["id"], $branch);
                    while ($checkrow = db_fetch($checkcredit)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_credit($myrow["id"], $myrow["reason_description"], $myrow["dissallow_invoices"], $myrow["inactive"], $branch);
                    }else {
                        add_to_credit($myrow["reason_description"], $myrow["dissallow_invoices"], $myrow["inactive"], $branch);
                    }
                }
                ...
                while ($myrow1 = db_fetch($result1)) {
                    ...
                    $checkincentive = check_incentive($myrow1["id"], $branch);
                    while ($checkrow = db_fetch($checkincentive)) {
                        $counter++;
                    }

                    if ($counter > 0) {
                        update_to_incentive($myrow1["id"], $myrow1["description"], $myrow1["module_type"], $myrow1["inactive"], $branch);
                    }else {
                        add_to_incentive($myrow1["description"], $myrow1["module_type"], $myrow1["inactive"], $branch);
                    }
                }              
           //}
           add_to_datalogs($currentdate, $remarks, $user_name);
