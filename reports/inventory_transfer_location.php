...
<title>INVENTORY TRANSFER LOCATION REPORT</title>
<style type="text/css">
	.main{
		width: 8.3in;
		height: 11in;
...
<?php
	...
	function get_FROM_location($reference)
	{
		set_global_connection();
		$sql = "SELECT SM.loc_code AS `code`, LOC.location_name AS `name`, SM.trans_no
				FROM `stock_moves` SM
					INNER JOIN `locations` LOC ON SM.loc_code = LOC.loc_code
				WHERE `reference` = '$reference' AND `qty` < '0'";

		return db_query($sql, "No transactions were returned");
	}
	function get_TO_location($reference)
	{
		set_global_connection();
		$sql = "SELECT SM.`loc_code` AS `code`, LOC.location_name AS `name`, SM.`reference`, SM.`tran_date` as `date`, SM.`category_id` as `category`, `memo_` AS `remarks`
				FROM `stock_moves` SM
					INNER JOIN `locations` LOC ON SM.loc_code = LOC.loc_code
					LEFT JOIN `comments` com ON SM.type = com.type AND SM.trans_no = com.id
				WHERE `reference` = '$reference' AND `qty` > '0'";

		return db_query($sql, "No transactions were returned");
	}
	
	function get_trans_location($trans_no,$type)
	{
		set_global_connection();
		$sql = "SELECT Smoves.qty as `qty`, Smoves.stock_id as `stock_id`, Smoves.color_code as `color_code`, Smoves.lot_no as `serial_no`, Smoves.chassis_no as `chassis_no`, Smoves.standard_cost as `unit_price`, Smaster.description as `description`
				FROM `stock_moves`  Smoves
					INNER JOIN `stock_master` Smaster ON Smoves.stock_id = Smaster.stock_id
				WHERE Smoves.`type` = '$type' AND Smoves.`trans_no` = '$trans_no' AND Smoves.`qty` > '0'";
	...
	//GET FROM LOCATION
	$FROM_result = get_FROM_location($reference);
	$FROM_row = db_fetch($FROM_result);  
	$from_location_code = $FROM_row["code"];
	$from_location_name = $FROM_row["name"];
	$trans_no = $FROM_row["trans_no"];

	//GET TO LOCATION
	$TO_result = get_TO_location($reference);
	$TO_row = db_fetch($TO_result);  
	$to_location_code = $TO_row["code"];
	$to_location_name = $TO_row["name"];
	$to_location_ref = $TO_row["reference"];
	$to_location_date = $TO_row["date"];
	$trans_category = $TO_row["category"];
	$Remarks = $TO_row["remarks"];
?>
...
<body>
	<div class="main printable">
		...
		<div style="width: 100%; text-align: center; padding-top: 0.2in; float: left; border-top: 0.1px solid black">
			...
			<h4>
				<div class="Merchandise">
					<label>Inventory Report Form - LOCATION TRANSFER</label>
				</div>			
			</h4>
		</div>
		<table id="header" cellspacing="0" cellpadding="0">
			<tr><td>&nbsp;</td></tr>
			<tr>
				<td align=left class="text-params">From Location:</td>
				<th style="width: 60%;" align=left><input type="text" value="<?php echo "$from_location_name"?>" class="underline_input_long" readonly></th>
				<td class="text-params">Reference #:</td>
				<th align=left><input type="text" value="<?php echo $to_location_ref?>" class="underline_input" readonly></th>
			</tr>
			<tr>
				<td align=left class="text-params">To Location:</td>
				<th style="width: 60%;" align=left><input type="text" value="<?php echo "$to_location_name"?>" class="underline_input_long" readonly></th>
				<td class="text-params">Trans date:</td>
				<th align=left><input type="text" value="<?php echo $to_location_date?>" class="underline_input" readonly></th>
			</tr>
			<tr><td>&nbsp;</td></tr>
		</table>
		<table style="width: 100%; border: 0px solid">
			<tr>
				...
				<th align=left><input style="width: 100%; colspan: 2;" type="text" value="<?php echo $Remarks?>" class="underline_input"></th>
			</tr>
		</table>
			...
			<div class="left" style="width: 100%; padding: 0px; float: left;">			
				<table style="width: 100%; float: left;" cellspacing="0" cellpadding="0">
					<tbody>						
						...
						<?php 			
							$result = get_trans_location($trans_no,$type);								
						...
						<?php
						  ...
						  $result = get_rr_supplier_gl($type_no,$type);				
							...
							//display_note(_("There are no line items on this dispatch."), 1, 2);	
								echo '<tr class="datatable">';	
								echo '<td colspan="5"; align=center style="border-right:0px solid; padding-left: 5px;">There are no GL account in this dispatch...</td>';
								echo '</tr>';
							*/
						?>-->
					</tbody>
