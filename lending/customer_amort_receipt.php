...
if(isset($_GET['get_aloc']))
{
    ...
    if($_GET['pay_type'] == "down"){
        ...
        if($_GET['colltype'] == 5){
            ...
            if($mos > 0){
                $value = check_transdate(date('Y/m/d', strtotime($schedrow["maturity_date"])), date('Y/m/d', strtotime($_GET['transdate'])));
                if($value == 1){
            ...
            $penalty_duedate = get_deptor_loan_penalty_sched($_GET['transNo'], $_GET['debtor_no'], $_GET['transtype']);

            $lpmos = date('m', strtotime($lastpayment_date));
            $lpyer = date('Y', strtotime($lastpayment_date));
            $duedate = date('d', date('Y/m/d', strtotime($schedrow["date_due"])));
            
            $lastpayment_date = date('Y/m/d', strtotime($lpyer.'-'.$lpmos.'-'.$duedate));

            $penltymos = mos_interval($lastpayment_date, $_GET['transdate']);

            $MonthNo = CalcMonthsDue_pnlty($_GET['transdate'], $penalty_duedate["date_due"], $schedrow["maturity_date"]);

            //check if pastdue account
            $pdvalue = check_transdate($schedrow["maturity_date"], $_GET['transdate']);
            if($pdvalue == 0){
                $PastDueNo = CalculateMonthsPastDue($_GET['transdate'], $penalty_duedate["date_due"], $schedrow["maturity_date"]);
                if(date('Y/m', strtotime($lastpayment_date)) != date('Y/m', strtotime($_GET['transdate']))){
...
if(isset($_GET['get_custPayment']))
{
    ...
    $result = get_debtor_payment_info(ST_CUSTPAYMENT, $_GET['module_type'], $start, $limit, $_GET['query'], 0, false, 0, $_GET['cashier']);
     //for pagination
...
if(isset($_GET['submit']))
{
    ...
    if ($InputError != 1){
        ...
        foreach($objDataGrid as $value=>$data) {
            ...
            $date_due = $schedrow["date_due"]; //$data['date_due'];
            ...
            if(($ar_balance - $total_rebate) == ($_POST['tenderd_amount'] + $_POST['total_otheramount'])){
                $islastPay = 1;
            ...
            //---->>>>>>> down payment -------------------------
                ...
                if($_POST['paylocation'] != "Lending"){
                    ...
                    if($_POST['total_amount'] == ($_POST['tenderd_amount'] + $_POST['total_otheramount'])){
                        add_loan_ledger($_POST['InvoiceNo'], $_POST['customername'], $Loansched_ID, $_POST['transtype'], ST_CUSTPAYMENT, ($_POST['tenderd_amount'] + $_POST['total_otheramount'] + check_isempty($dp_discount)), 0, 0, 0, $trans_date, $payment_no);
                    ...
                    }elseif($_POST['total_amount'] > ($_POST['tenderd_amount'] + $_POST['total_otheramount'])) {
                        $nextDPBal = ($_POST['total_amount'] - ($_POST['tenderd_amount'] + $_POST['total_otheramount']));
            ...
            //---->>>>>>> amortization payment -------------------------
                ...
                if($_POST['collectType'] == 5){
                    ...
                    */
                    ...
                    //if(isTermFullpayment($_POST['InvoiceNo'], $_POST['transtype'], $_POST['customername']) == 1){
                    if($termoderow['term_mode_fullpayment'] == 1){
    ...
    if ($InputError != 1){
        ...
        foreach($objDataGrid as $value=>$data) {
            ...
            if(!empty($data['gl_code'])){
                ...
                if($data['sl_code'] == $_POST['customername_inb']){
                    ...
                    add_gl_trans(ST_CUSTPAYMENT, $payment_no, $_POST['trans_date_inb'], $data['gl_code'], 0, 0, '', $amount, $bank['bank_curr_code'], PT_CUSTOMER, $_POST['customername_inb'], '', 0, $data['sl_code'], $data['sl_name'], 0, 0);
                }
            }
        }
        //commit_transaction();
        ...
        if(!empty($company_prefs["deployment_status"])){
            interbranch_send_payment_add($_POST['branch_inb'], $interb_ref, $_POST['name_inb'], $trans_date, $_POST['ref_no_inb'], $_POST['total_amount_inb'],
                                            $_POST['remarks_inb'], $_POST['preparedby_inb'], $conn['branch_code'], $payment_no, ST_CUSTPAYMENT, $_POST['branch_inb'], $_POST['moduletype_inb']);
            ...
            $islive = get_islive($_POST['branch_inb']);
